# -*- coding: utf-8 -*-
"""
Created on Sat Mar  7 17:10:40 2020
...
# In[204]:
optimizer_chosen = np.array(['adam', 'adagrad','sgd'])
plt.close('all')
...
for i in np.arange(0,3):
    ...
    model.compile(optimizer=optimizer_chosen[i], loss='mse', metrics=['accuracy'])
...
# In[210]:
    ...
    plt.suptitle(optimizer_chosen[i])
    plt.ylabel('loss')
    plt.xlabel('epoch')
    plt.ylim([-0.1,0.7])
...
# In[211]:
    ...
    plt.ylim([-1.5,1.5])
    _ = plt.plot([-2,2], [-2,2])
    plt.title(optimizer_chosen[i], fontsize=14, fontweight='bold')
    plt.show()
